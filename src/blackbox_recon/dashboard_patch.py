...
from .operator_progress import progress_enabled, with_heartbeat
...
def _install_recon_rprint_suppressor() -> None:
    ...
    try:
        ...
        pass
    recon_mod.print_execution_recap = lambda results, *, echo=True: None
    ...
    def _make_wrapper(fn: Any, label: str):
        ...
        async def wrapped(*args: Any, **kwargs: Any) -> Any:
            ...
            if label == "CMS/app-aware enumeration":
                detail = "CMS probes/WPScan may take a few minutes on lab targets"
            elif label == "Vulnerability intelligence":
                detail = "checking SearchSploit and NVD/NIST candidate CVE leads"
            try:
                result = await with_heartbeat(label, fn(*args, **kwargs), detail=detail)
...
def patch_operator_dashboard(ReconEngine: Any) -> None:
    ...
    async def run_with_operator_dashboard(self: Any, target: str, modules: List[str]) -> Dict[str, Any]:
        ...
        results = await with_heartbeat("Core scan", original_run(self, target, modules), detail="Nmap/content discovery may be quiet for a while")
        if progress_enabled():
